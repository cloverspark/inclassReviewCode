            ...
            if (next.equals(")")) {
                if (symbols.size() < 2 || symbols.peek().equals("(")) {
                    ...
                    if (!symbols.peek().equals("(")) {
                        error = true;
                    } else {//its popping the ()'s off
                        symbols.pop(); 
                        double oper2 = values.pop();//It pops the numbers off as Values
                        double oper1 = values.pop();//It pops the numbers off as Values
                        double value = evaluate(operator, oper1, oper2); //call method that does calculation
                        values.push(value); //returns value
                    }
                }
